...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    ...
    val others = this.remove(mostRetweeted)
    if (others.isEmpty) 
      new Cons(mostRetweeted, Nil)
    else
      new Cons(mostRetweeted, others.descendingByRetweet)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    if (!left.isEmpty) {
      ...
      if (acc.retweets < leftmost.retweets)
         acc = leftmost
    }
    if (!right.isEmpty) {
     val rightmost = right.mostRetweeted
      if (acc.retweets < rightmost.retweets)
         acc = rightmost
    }
    acc
  }    
